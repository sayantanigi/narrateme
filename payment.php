...
print_r($_SESSION["cart_item"]);
    				...
    				$item_total = 0;
					echo $member_id=$_SESSION['email_address'];
					...
					foreach ($_SESSION["cart_item"] as $item){
						...
						$total = $item["quantity"]*$item["price"];
		 				...
		 				$sqlinsert=mysql_query("INSERT INTO order_list (transaction_id, product_code, product_name, qty, price, total, member_id, add_date, pay_status) 
						VALUES ('".$transantionid."','".$item["code"]."', '".$item['product_name']."', ".$item["quantity"].", ".$item["price"].", ".$total.", '".$member_id."', '".date('Y-m-d')."', '0')");
		
					}
					
					$sqlinsertship=mysql_query("INSERT INTO shipping (transaction_id, first_name, last_name, country, state, city, address, zipcode, phone_no, customer_id) 
					VALUES ('".$transantionid."', '".$_SESSION['first_name']."','".mysql_real_escape_string($_SESSION['last_name'])."','".$_SESSION['country_id']."', '".$_SESSION['state']."','".$_SESSION['city']."','".$_SESSION['address']."','".$_SESSION['zip_code']."', '".$_SESSION['phone_number']."', '".$member_id."')");
					}
				 ?>
