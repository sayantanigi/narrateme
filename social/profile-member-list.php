...
if(isset($_REQUEST['unfriend'])){
	...
	$result=mysql_query("DELETE from `na_frnd_reqst` where (`from_id`= ".$_SESSION["userid"]." || `to_id`= ".$_SESSION["userid"].") and (`from_id`=".$_REQUEST['from_id']." || `to_id`=".$_REQUEST['from_id'].") and `frnd_status`=2");
	if($resultupdatefrnd){
          ...
          <div class="card wall-posting">
            <div class="p-header">
              <ul class="p-menu">
                ...
                <!--<li class="pm-search">
                  <div class="pms-inner"> <i class="zmdi zmdi-search"></i>
                    <input type="text" placeholder="Search..." id="skills">
                    <input type="submit" name="searchmemsocial" value="Search">
                  </div>
                </li>-->
              </ul>
          ...
          <div class="pmb-block">
			...
			if(mysql_num_rows($frndlst)>0){
				while($rowlist=mysql_fetch_array($frndlst)){
					...
					//print_r($viewmemberfrnd);
			?>
              <div class="col-md-3 col-sm-4 col-xs-6">
                <div class="c-item"> <a href="member_list_profile.php?searchmember=<?php echo $viewmemberfrnd['id']?>" class="ci-avatar"> 
                	<?php
